...
@section('page-title', 'Point of Sale - Restaurant')

@section('sidebar')
    <a href="{{ route('kasir.restoran.dashboard') }}" class="nav-link">
        <i class="bi bi-speedometer2"></i> Dashboard
    </a>
    <a href="{{ route('kasir.restoran.pos') }}" class="nav-link active">
...
<div class="row g-4">
    ...
    <div class="col-lg-7">
        ...
        {{-- Menu Grid dengan Data Dummy yang Lengkap & Banyak --}}
        <div class="row g-3" id="menuGrid">
            ...
            @foreach($simulatedMenus as $menu)
            <div class="col-sm-6 col-md-4 menu-item" data-category="{{ $menu['category'] }}">
                <div class="card h-100 pos-card-menu" onclick="tambahKeKeranjangLokal({{ $menu['id'] }}, '{{ $menu['name'] }}', {{ $menu['price'] }})">
                    <div style="position: relative; overflow: hidden;">
                        <span class="badge-category">{{ $menu['category'] }}</span>
                        <img src="{{ $menu['img'] }}" class="card-img-top" alt="{{ $menu['name'] }}" style="height: 120px; object-fit: cover;">
                    </div>
                    <div class="card-body p-3 d-flex flex-column justify-content-between">
                        <div>
                            <h6 class="card-title fw-bold mb-1 text-truncate" style="color: #2c2520;" title="{{ $menu['name'] }}">{{ $menu['name'] }}</h6>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mt-2">
                            <span class="fw-bold fs-6 text-gold-price">Rp {{ number_format($menu['price'], 0, ',', '.') }}</span>
        ...
        <div class="card cart-panel">
            ...
            <div class="card-body p-3">
                <div class="cart-container" id="cartItemsContainer">
                    {{-- Diisi secara realtime via JavaScript --}}
        ...
        <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
            ...
            <div class="modal-body p-4" style="background: #fcfbfa;">
                ...
                <div id="modal-bank-section" style="display:none; background: #fff; padding: 15px; border-radius: 10px; border: 1px solid #f2ede7; margin-bottom: 15px;">
                    ...
                    <input type="text" class="form-control form-control-sm" value="TRX-POS{{ rand(100,999) }}" style="border-color: #d4b795;">
                </div>
...
{{-- REALTIME CLIENT-SIDE SCRIPT CONTROL (NO RELOAD ERROR 404) --}}
<script>
let keranjangLokal = {};
