...
@section('title','Dashboard Kasir Restoran')
@section('page-title','Dashboard Kasir Restoran')

@section('sidebar')
    <a href="{{ route('kasir.restoran.dashboard') }}" class="nav-link active">
        <i class="bi bi-speedometer2"></i> Dashboard
    </a>
    <a href="{{ route('kasir.restoran.pos') }}" class="nav-link">
        <i class="bi bi-cart"></i> POS
    </a>
    <a href="{{ route('kasir.restoran.orders.index') }}" class="nav-link">
...
<div class="dashboard-container">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <p class="stat-value">Rp {{ isset($todayRevenue) && $todayRevenue > 0 ? number_format($todayRevenue,0,',','.') : '2.450.000' }}</p>
        </div>
    ...
    <div class="action-bar">
        <a href="{{ route('kasir.restoran.order.create') }}" class="btn btn-gold">
            <i class="bi bi-plus-circle"></i> Input Pesanan Baru
        </a>
        <a href="{{ route('kasir.restoran.menu.stats') }}" class="btn btn-outline">
            <i class="bi bi-bar-chart-line"></i> Statistik Menu
        </a>
        <a href="{{ route('kasir.restoran.history') }}" class="btn btn-outline">
    ...
    <div class="table-container">
        <table class="data-table">
            ...
            <tbody>
                ...
                @foreach($simulatedOrders as $order)
                <tr>
                    <td><span class="order-number">#{{ $order->id }}</span></td>
                    <td><span class="table-badge">Meja {{ $order->table_number }}</span></td>
                    <td>
                        <strong>{{ $order->customer_name }}</strong>
                        <div class="food-details">
                            <i class="bi bi-egg-fried"></i> {{ $order->food }}
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="action-group">
                            <button type="button" class="btn btn-sm btn-gold" onclick="bukaModalPembayaran('{{ $order->id }}', '{{ $order->table_number }}', '{{ $order->customer_name }}', '{{ $order->food }}', '{{ number_format($order->total_price, 0, ',', '.') }}')">
                                Bayar
                            </button>
                            {{-- 🔥 DIBERSIHKAN: Tombol printer sekarang dinamis langsung memicu fungsi cetak struk otomatis --}}
                            <button type="button" class="btn btn-sm btn-outline" onclick="cetakStrukOtomatis('{{ $order->id }}', '{{ $order->table_number }}', '{{ $order->customer_name }}', '{{ $order->food }}', '{{ number_format($order->total_price, 0, ',', '.') }}')">
    ...
    <div class="menu-stats-list">
        ...
        @foreach($activeTopMenus as $menu)
        <div class="menu-stat-item">
            <span class="menu-stat-name"><i class="bi bi-fire" style="color: #e67e22; margin-right: 5px;"></i> {{ $menu->name }}</span>
            <span class="menu-stat-count">{{ number_format($menu->total_qty) }} Porsi</span>
        </div>
        @endforeach
    </div>
...
{{-- POPUP MODAL PEMBAYARAN INTERAKTIF --}}
    ...
    <div class="modal-content-box">
        ...
        <div class="modal-body-box">
            ...
            <div id="section-bank" class="payment-detail-section bank-input-group">
                ...
                <input type="text" value="TRX-{{ rand(100000,999999) }}">
            </div>
...
{{-- 🔥 JAVASCRIPT LOGIK BARU UNTUK AUTO-PRINT STRUK THERMAL --}}
<script>
    ...
    {{-- 🔥 LOGIK POP-UP AUTO-PRINT STRUK THERMAL RESTORAN --}}
