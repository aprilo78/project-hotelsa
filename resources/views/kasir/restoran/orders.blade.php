...
@section('sidebar')
    ...
    <a href="{{ route('kasir.restoran.orders.index') }}" class="nav-link active">
    ...
    <div class="table-container">
        <table class="data-table" id="ordersTable">
            ...
            <tbody>
                ...
                @foreach($simulatedOrders as $o)
                <tr class="order-row-item" data-tanggal="{{ $o['date'] }}" data-status="{{ $o['status'] }}">
                    <td class="ps-4"><span class="order-id-badge">#{{ $o['id'] }}</span></td>
                    <td>
                        <div class="fw-bold text-dark">{{ $o['guest_name'] }}</div>
                        <span class="table-badge-room"><i class="bi bi-door-open"></i> {{ $o['room'] }}</span>
                    </td>
                    <td>
                        <span class="text-muted" style="font-size:13px;"><i class="bi bi-egg-fried"></i> {{ $o['items'] }}</span>
                    </td>
                    <td>
                        <strong style="color: #2c2520;">Rp {{ number_format($o['total'], 0, ',', '.') }}</strong>
                    </td>
                    <td>
                        @if($o['status'] == 'paid')
                    ...
                    <td>
                        <div class="text-dark fw-semibold" style="font-size:13px;">{{ date('d M Y', strtotime($o['date'])) }}</div>
                        <small class="text-muted" style="font-size:11px;"><i class="bi bi-clock"></i> Pukul {{ $o['time'] }} WIB</small>
                    </td>
                </tr>
                @endforeach
                
                {{-- Row kosong fallback jika filter tidak menemukan data --}}
...
{{-- JAVASCRIPT LOGIK FILTER DATA ORDERS HOTEL REALTIME --}}
<script>
