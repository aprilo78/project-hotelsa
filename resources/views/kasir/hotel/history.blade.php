...
@section('sidebar')
    ...
    <a href="{{ route('kasir.hotel.invoices.index') }}"
       class="nav-link {{ request()->routeIs('kasir.hotel.invoices.*') ? 'active' : '' }}">
...
{{-- ── KONTEN UTAMA DENGAN LOGIKA FILTER OTOMATIS ── --}}
@section('content')
@php
...
<div class="container-fluid px-0">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="date" name="from" class="form-control bg-light border-start-0 ps-0" value="{{ request('from') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <span class="input-group-text bg-light text-muted border-end-0"><i class="bi bi-calendar3"></i></span>
                            <input type="date" name="to" class="form-control bg-light border-start-0 ps-0" value="{{ request('to') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <div class="w-100 d-flex gap-2">
                            ...
                            @if(request('from') || request('to'))
                                <a href="{{ route('kasir.hotel.transactions.history') }}" class="btn btn-light border px-3" title="Reset Filter">
    ...
    <div class="alert alert-lux-revenue p-3 mb-4 d-flex align-items-center justify-content-between shadow-sm">
        <div>
            ...
            <span>Total Pemasukan Kasir <strong>{{ request('from') ? '(Periode Terpilih)' : '(Seluruh Periode)' }}</strong> :</span>
        </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            {{-- 4. Nama Kasir --}}
                            <td>
                                <span class="text-secondary"><i class="bi bi-person-badge me-1"></i>{{ $p->kasir->name ?? '-' }}</span>
                            </td>
                            ...
                            <td class="text-secondary font-monospace">
                                {{ is_string($p->created_at) ? $p->display_date : $p->created_at->format('d M Y') }}
                            </td>
                        </tr>
                        @empty
                        <tr>
