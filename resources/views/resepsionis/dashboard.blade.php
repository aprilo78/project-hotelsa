    ...
    <div class="row mb-4">
        ...
        <div class="col-md-3 mb-3">
            <div class="card lux-card card-info p-3">
                ...
                <h2>{{ count($todayCheckInsSimulation) }}</h2>
                <small>Tamu</small>
            </div>
        </div>
        ...
        <div class="col-md-3 mb-3">
            <div class="card lux-card card-danger p-3">
                ...
                <h2>{{ count($todayCheckOutsSimulation) }}</h2>
                <small>Tamu</small>
            </div>
        </div>
        ...
        <div class="col-md-4">
            ...
            <div class="card lux-card mb-3 p-3">
                ...
                <div style="max-height:300px; overflow-y:auto;">
                    @forelse($todayCheckInsSimulation as $booking)
                        <div class="border-bottom py-2 d-flex justify-content-between align-items-center">
                            <div>
                                <strong>{{ $booking->user->name }}</strong><br>
                                <small class="text-muted">Kamar {{ $booking->room->room_number }}</small>
                            </div>
                            <button class="btn btn-sm btn-lux" onclick="checkIn({{ $booking->id }})">
                                Check-in
                            </button>
                        </div>
                    @empty
            ...
            <div class="card lux-card p-3">
                ...
                <div style="max-height:300px; overflow-y:auto;">
                    @forelse($todayCheckOutsSimulation as $booking)
                        <div class="border-bottom py-2 d-flex justify-content-between align-items-center">
                            <div>
                                <strong>{{ $booking->user->name }}</strong><br>
                                <small class="text-muted">Kamar {{ $booking->room->room_number }}</small><br>
                                <small class="text-success fw-bold">
                                    Sisa: Rp {{ number_format($booking->remaining_payment, 0, ',', '.') }}
                                </small>
                            </div>
    ...
    <div class="card lux-card p-3 mb-4">
        ...
        <div class="table-responsive">
            <table class="table align-middle table-hover">
                ...
                <tbody>
                @forelse($roomServiceOrdersSimulation as $order)
                    <tr>
                        <td><span class="badge bg-secondary">Room {{ $order->room_number }}</span></td>
                        <td>
                            @foreach($order->items as $item)
                                <strong>{{ $item->menu->name }}</strong><br>
                            @endforeach
                        </td>
                        <td>{{ data_get($order->items, '0.quantity', 1) }} Porsi</td>
                        <td><span class="text-dark fw-bold">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span></td>
                        <td><code>{{ $order->created_at->format('H:i') }} WIB</code></td>
                        <td>
                            <button class="btn btn-sm btn-lux" onclick="tagToBill({{ $order->id }})">
