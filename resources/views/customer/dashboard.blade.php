...
{{-- SIMULASI / FALLBACK DATA AGAR DASHBOARD LANGSUNG TERLIHAT PADAT & AKTIF --}}
@php
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <small class="text-muted d-block">{{ $booking->room->roomType->name ?? '' }}</small>
                            </td>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" id="dashCheckIn" class="form-control" min="{{ date('Y-m-d') }}">
                </div>

                <div class="mb-3">
                    <label class="form-label small fw-semibold text-muted">Tanggal Check-out</label>
                    <input type="date" id="dashCheckOut" class="form-control" min="{{ date('Y-m-d', strtotime('+1 day')) }}">
                </div>
...
function searchRooms() {
    ...
    const url = `{{ route('customer.search-rooms') }}?check_in=${checkIn}&check_out=${checkOut}&guests=${guests}&ajax=1`;
    ...
    fetch(url, { headers:{ 'X-Requested-With':'XMLHttpRequest' } })
        ...
        .catch(() => {
            ...
            window.location.href = `{{ route('customer.search-rooms') }}?check_in=${checkIn}&check_out=${checkOut}&guests=${guests}`;
        })
        .finally(() => {
    ...
    list.innerHTML = rooms.map(r => {
        ...
        <div class="col-md-6">
          <div class="room-result-card p-3">
            ...
            <a href="{{ route('customer.new-booking') }}?room_id=${r.id}&check_in=${checkIn}&check_out=${checkOut}&guests=${guests}"
