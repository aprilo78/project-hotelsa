...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    ...
    <a href="{{ route('customer.new-booking') }}" class="btn btn-gold">
        + Booking Baru
    </a>
</div>
...
<div class="mb-4 p-3" style="background:#f5efe6;border-radius:12px;border:1px solid #ede8dc">
    <div class="d-flex gap-2 flex-wrap">
        ...
        <a href="{{ request()->fullUrlWithQuery(['status' => $val]) }}"
           class="filter-btn {{ $current === $val ? 'active' : '' }}">
            {{ $lbl }}
        </a>
        @endforeach
    </div>
...
<div class="booking-card text-center py-5">
    ...
    <h5 style="font-family:'Playfair Display',serif;color:#1A1A2E">
        Belum ada booking dengan status "{{ $statuses[$current] ?? $current }}"
    </h5>
    <p class="text-muted">Mulai reservasi kamar Anda sekarang</p>
    <a href="{{ route('customer.new-booking') }}" class="btn btn-gold mt-1">Booking Sekarang</a>
</div>
@else
...
<div class="booking-card p-3 p-md-4" id="card-{{ $booking->id }}">
    ...
    <div class="d-flex justify-content-between flex-wrap gap-2">
        <div>
            ...
            <div class="text-muted" style="font-size:.78rem">
                Booking #{{ $booking->id }} · {{ $booking->created_at->diffForHumans() }}
            </div>
            <div class="mt-2 d-flex gap-2 flex-wrap">
                <span class="badge-soft badge-status" style="background:{{ $sc['bg'] }};color:{{ $sc['color'] }}">
                    {{ $sc['label'] }}
                </span>
                <span class="badge-soft badge-payment" style="background:{{ $pc['bg'] }};color:{{ $pc['color'] }}">
                    {{ $pc['label'] }}
                </span>
            </div>
        </div>
        <div class="text-end">
            <div class="price" data-total="{{ $booking->total_price }}">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</div>
            <div class="text-muted" style="font-size:.78rem">{{ $nights }} malam</div>
        </div>
    </div>
    ...
    <div class="row g-3">
        <div class="col-6 col-md-3">
            ...
            <div style="font-size:.88rem">{{ $booking->check_in->format('d M Y') }}</div>
        </div>
        <div class="col-6 col-md-3">
            <div class="label">Check-out</div>
            <div style="font-size:.88rem">{{ $booking->check_out->format('d M Y') }}</div>
        </div>
        <div class="col-6 col-md-3">
            <div class="label">Sudah Dibayar</div>
            <div class="paid-text" style="color:#065f46;font-size:.88rem">
                Rp {{ number_format($totPaid, 0, ',', '.') }}
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="label">Sisa</div>
            <div class="balance-text" style="color:{{ $remBal > 0 ? '#991b1b' : '#065f46' }};font-size:.88rem">
                Rp {{ number_format($remBal, 0, ',', '.') }}
            </div>
        </div>
    </div>
    ...
    <div class="mt-3 d-flex gap-2 flex-wrap action-container">
        <a href="{{ route('customer.booking-detail', $booking->id) }}" class="btn btn-sm" style="background:#1A1A2E;color:#fff;border-radius:8px">
            Detail
        </a>

        @if(in_array($booking->status, ['pending','confirmed']))
        <button onclick="prosesBatal({{ $booking->id }})" class="btn btn-sm btn-batal" style="background:#FEE2E2;color:#991B1B;border-radius:8px;border:none">
            Batalkan
        </button>
        @endif

        @if($booking->status === 'checkout')
        <a href="{{ route('customer.booking-review', $booking->id) }}" class="btn btn-sm" style="background:#EDE9FE;color:#4C1D95;border-radius:8px">
            Beri Ulasan
        </a>
        @endif

        {{-- Perubahan di Sini: Ganti tag <a> menjadi <button> dengan fungsi klik langsung tanpa ganti halaman --}}
        @if(in_array($booking->status, ['pending','confirmed']) && $remBal > 0)
        <button onclick="prosesBayar({{ $booking->id }})" class="btn btn-sm btn-gold btn-bayar">
            Bayar Sekarang
        </button>
        @endif
    </div>
...
{{-- JAVASCRIPT SIMULASI SATU TEMPAT --}}
<script>
